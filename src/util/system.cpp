...
const char * const BITCOIN_CONF_FILENAME = "altercoin.conf";
...
#else
    const char* pszModule = "altercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Altercoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Altercoin";
#else
    // Unix
    return pathRet / ".altercoin";
#endif
#endif
}
